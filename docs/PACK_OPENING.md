...
- `acquisitionState` replica esa decision como `NEW` o `DUPLICATE` para UI.
...
- `duplicateQuantityAfter = max(quantityAfter - 1, 0)`.
- `canPaste` indica si esa figurita queda disponible para pegar en el album.
